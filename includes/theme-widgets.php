...
function widget_Twidget_init() {
	...
	function widget_Twidget($args) {
		...
		echo '<ul id="twitter_update_list"><li></li></ul><div class="follow"><a href="http://www.twitter.com/'.$account.'/" title="'. __('Follow us on Twitter',woothemes) .'">'. __('Follow us on Twitter',woothemes) .'</a></div>
...
<div class="widget flickr">
	<h3 id="flickr"><?php _e('Flickr Photos',woothemes); ?></h3>
...
?> 
	...
	<div class="widget subscribe">	
		...
		<form action="<?php if ($google) { ?>http://feedburner.google.com/fb/a/mailverify<?php } else { ?>http://www.feedburner.com/fb/a/emailverify<?php } ?>" method="post" target="popupwindow" onsubmit="window.open('<?php if ($google) { ?>http://feedburner.google.com/fb/a/mailverify?uri=<?php } else { ?>http://www.feedburner.com/fb/a/emailverifySubmit?feedId=<?php } ?><?php echo $id; ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
			...
			<p class="fields">
					<input class="field text" type="text" name="email" value="<?php _e('Enter your email address',woothemes); ?>" onfocus="if (this.value == '<?php _e('Flickr Photos',woothemes); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e('Flickr Photos',woothemes); ?>';}" />
					<input type="hidden" value="<?php echo $id; ?>" name="uri"/>
					<input type="hidden" value="<?php bloginfo('name'); ?>" name="title"/>
					<input type="hidden" name="loc" value="en_US"/>
				<button class="replace" type="submit"><?php _e('Sign',woothemes); ?></button>
			</p>
...
?>
   ...
   <h3 id="photos"><?php echo $text; ?></h3>
        <ul class="news">
             ...
             $posts = get_posts('cat='.$blog_cat.'&showposts='.$amount); 
